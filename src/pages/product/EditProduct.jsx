...
const EditProduct = () => {
  ...
  const [oldGallery, setOlaGallery] = useState([]);
  ...
  const handleChangeGallery = (e, index) => {
    setDataRemove((prev) => {
      const newValue = [...prev];
      newValue.push(oldGallery[index]);
      return newValue;
    });
  ...
  const handleRemoveImage = (index, index2) => {
    setDataRemove((prev) => {
      const newValue = [...prev];
      newValue.push(color[index].gallery[index2].preview);
      return newValue;
    });
  ...
  useEffect(() => {
    const detail = dataProduct.detail;

    if (detail) {
      let sizes = detail.map((item) => item.sizeId);
      sizes = [...new Set(sizes)];

      const index = sizes.map((item) => {
        return dataSize.findIndex((item2) => item2.id === item);
      });
      setValue("size", index);
      setSize(index);
      setOldSize(sizes);
    }
  }, [dataSize, dataProduct]);
  useEffect(() => {
  ...
  useEffect(() => {
    api.get(`/product/get-one/${id}`).then((res) => {
      setDataProduct(res.data.data);
      setValue("nameProduct", res.data.data.nameProduct);
      setValue("categoryId", res.data.data.categoryId);
      setValue("regularPrice", res.data.data.regularPrice);
      setValue("salePrice", res.data.data.salePrice);
      setValue("weight", res.data.data.weight);
      setValue("dimensions", res.data.data.dimensions);
      setValue("materials", res.data.data.materials);
      setValue("other", res.data.data.other);
      setValue("subDesc", res.data.data.subDesc);
      setValue("desc", res.data.data.desc);
      setValue("quantity", res.data.data.detail[0].quantity);
      setOlaGallery(() => {
        return res.data.data.gallery.map((item) => item.listLink);
      });
      setOldColor((prev) => {
        const newValue = [...prev];
        res.data.data.gallery.map((item, index) => {
          newValue.push(item.colorId);
        });
        return newValue;
      });
      setColor((prev) => {
        const newValue = [];
        res.data.data.gallery.map((item, index) => {
          const newItem = { color: "", gallery: [{ preview: "" }] };
          newItem.color = item.colorId;
          newItem.gallery = item.listLink.split(",").map((item2) => {
            return { preview: item2 };
          });
    ...
    >
      ...
      <form
        onSubmit={handleSubmit((data) => {
          ...
          formDataAddProduct.append("oldGallery", oldGallery);
          formDataAddProduct.append("dataRemove", dataRemove);
          formDataAddProduct.append("oldColor", oldColor);
          formDataAddProduct.append("oldSize", oldSize);

          api.put(`/product/update/${id}`, formDataAddProduct).then((res) => {
